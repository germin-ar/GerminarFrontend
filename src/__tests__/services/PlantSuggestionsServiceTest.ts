...
describe("PlantSuggestionServiceTest", () => {

    afterEach(() => {
        jest.clearAllMocks();
    });

    it('test get from PlantSuggestion ok', async () => {
        ...
        const responseBody = [{
            ...
            "slug_scientific_name": "thymus-vulgaris",
            "genus": "Tomillos",
        ...
        const service:PlantSuggestionService = new PlantSuggestionService("")

        const result: PlantSuggestion [] = await service.getPlantsSuggestion("11", "111", "10", "12")

        expect(result.at(0)?.id).toEqual(3);
    })


    it('test get from PlantSuggestion with error response', async () => {
        global.fetch = jest.fn(() =>
            Promise.reject({
        ...
        await expect(service.getPlantsSuggestion("11","111","2","12")).rejects.toBeDefined()
    })
    it('test get from PlantsSuggestionPlace ok', async () => {
        ...
        const result: PlantSuggestion [] = await service.getPLantsSuggestionPlace("11", "invierno", "10")
    ...
    it('test get from PlantsSuggestionPlace with error response', async () => {
        ...
        await expect(service.getPLantsSuggestionPlace("10","invierno","2")).rejects.toBeDefined()
    })
